...
import py2exe
from boatman import (NAME, VERSION, DESC, LONG_DESC, LICENSE, URL, AUTHOR,
                     EMAIL, KEYWORDS, CLASSIFIERS, SCRIPT, DATA_FILES,
                     DATA_FILES_PY2EXE)


#PACKAGES = [NAME] # only used if find_packages() does not work

ENTRY_POINTS = {'console_scripts': ['boatman=boatman.boatman:main'],
                #gui_scripts=['app_gui=boatman.boatman:start']
                }

PKG_DATA = dict(boatman=DATA_FILES)
##PKG_DATA = {'': ['*.txt', '*.rst'], 'boatman': ['*.txt'],
##            'boatman.data': ['*.pkl']}
...
setup(name=NAME,
      ...
      #windows=[{'script': 'boatman.py',
      #          'icon_resources': [(0, 'boatman.ico')]
      #          }],
      #zipfile=None
      )
